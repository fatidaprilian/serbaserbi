...
1. **Test Guest Mode (Zero Login Required):**
   * Navigate to `http://localhost:3000` and click the **"Create Invoice"** card.
   * Switch the currency dropdown to **USD** -> observe the **"Cached Rate (12h TTL)"** badge and automatic conversion (*Caching Logic*).
   ...
   * Click the **"Print / Download PDF"** button -> the bilingual PDF document is rendered immediately in your browser viewport.
...
3. **Audit Cash Flow & Record Installment / Down Payment:**
   ...
   * Click **"Record Payment"** -> record a final settlement of Rp4,000,000 -> observe the invoice status automatically transition to **Paid** in real time (*Database & API Endpoints*).
4. **Trigger Background Job & Overdue Reminders:**
   * On the Documents page, click the **"Check Overdue"** button in the top toolbar.
